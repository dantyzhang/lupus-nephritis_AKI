...
```{r 00_install packages& citation}
rm(list = ls())
...
```{r 01_raw data_trian1_test1}
rm(list = ls())

# data1<-read.csv("SLEAKI20210103_fionafinal.csv",sep=",",header=TRUE)
# vis_miss(data1)

data<-read.csv("SLEAKI20210327.csv",sep=",",header=TRUE)
...
vars <- c(
  ...
  "SSA", "RIBP", "nRNP", "Sm", "Au","dsDNAbi", "acl",#factor#auto-anti SSB elevatetopluse
  "organ" ,"duration_2mg" ,"crrt", "pe" , #new factor
...
factorvars<-c("Center","gender","IS", "response",  "DIE", "esrd" ,"HTN",#epi_clinic
  "SSA", "RIBP", "nRNP", "Sm", "Au","dsDNAbi", "acl", #auto-anti
   "crrt", "pe" , #new factor
   "biopsy" ,"typeofln2","typeofln3" , "typeofln4" , "TMA","TIsclerosis", "tiinflammation","leukocyte_infiltration", #histo
...
```{r 02_test of normality--shapiro-wilk_oridat}
# http://www.360doc.com/content/18/0408/11/42030643_743743490.shtml
...
```{r 02_table1, eval=FALSE, include=FALSE}
#---------------------1.table1_center---------------------
library("tableone")
a<-data
attach(a)
...
```{r 02_table1-mean-sd, eval=FALSE, include=FALSE}
#---------------------1.table1_center---------------------
library("tableone")
a<-data
attach(a)
...
vars_need<-c("response",# "recovertime",
  ...
  "ALB", "CR", "UP24H","BNP","ESR", "CRP", "IGG", "C3", "C4", "HB", "PLT","DD","RBC",  #numeric#lab 
  "SSA", "RIBP", "nRNP", "Sm", "Au","dsDNAbi", "acl",#"SSB",                                 #factor#auto-anti 
  "organ" ,"duration_2mg" ,"crrt", "pe" , #new factor
  "cellularcrescent", "fibrouscrescent", "glomerularsclerosis", "TIatrophy", #"AI", "CI"            #numeric#histo 
...
#-----2.imputation of variables with less than 6% missing values in train set(without imputation histological variables)--------------------
trainneed<-train1[,vars_need];vis_miss(trainneed);str(trainneed)  ##n=130  v=43
names(trainneed)
trainnobiopsy<-trainneed[,1:32];vis_miss(trainnobiopsy)##32
trainbiopsy<-trainneed[,33:43];vis_miss(trainbiopsy)#11 
...
#---------3.imputation of variables with less than 11% missing values in test set(without imputation histological variables)------
testneed<-test1[,vars_need];vis_miss(testneed);str(testneed)  ##n=96  v=43

testnobiopsy<-testneed[,1:32];vis_miss(testnobiopsy)##29 ##missing values in BNP were 80%
BNP<-testnobiopsy$BNP
testbiopsy<-testneed[,33:43];vis_miss(testbiopsy)##
...
```{r 04_datpre_table1_impudat, eval=FALSE, include=FALSE}
...
```{r 04_table1_impudat, eval=FALSE, include=FALSE}
#---------------------1.table1_center---------------------
library("tableone")
a<-allpre
attach(a)
...
```{r 05_data pre for lasso}
rm(list = ls())
load("train_mice.Rdata");vis_miss(trainpre)
load("test_mice.Rdata");vis_miss(testpre)
trainbio<-na.omit(trainpre);vis_miss(trainbio) #n=82
names(trainpre)
trainnobio<-trainpre[,1:32];vis_miss(trainnobio) #n=130

names(testpre);vis_miss(testpre)
testbio<-na.omit(testpre[,-c(12,40:43)]);vis_miss(testbio) ## n=73
testnobio<-testpre[,c(1:11,13:32)];vis_miss(testnobio) ##n=96
```


```{r 06_train_lasso_ori_8histo}
#--------------train--AKI model with 8 histological variables-----------------
train<-trainbio;vis_miss(train);str(train)  #with biopsy  # n=82 v=43
for(i in names(train)[c(1:43)]) {train[,i] <- as.numeric(train[,i])}    #numeric
library("glmnet")
set.seed(123) 

x <- as.matrix(train[,2:43])
y <- train[, c("response")]
...
tiinflammation          .          
leukocyte_infiltration  .          
karyorrhexis            .          
wire_loop               .          
fibrinoid               .          

```{r 07_train_lasso_ori_without_8histo}
#-------------test--AKI model without histological variables---------------
train<-trainnobio  # n=82 v=31
for(i in names(train)[c(1:32)]) {train[,i] <- as.numeric(train[,i])}
...
Measure: Misclassification Error 
     ...
     Lambda Measure      SE Nonzero
min 0.02103  0.2154 0.04299      14
1se 0.09317  0.2539 0.04690       5

1se：
sleduration       -0.018319661
elevatedcrtotreat -0.018135775
CR                -0.003295871
C4                -1.450025014
crrt              -0.384307120
...
```{r 09_train creat new cutoff by OptimalCutpoints}
#add col
...
```{r 10_test creat new cutoff by OptimalCutpoints}
...
#-------------5.build AKI_bio model with logi regression--------------------------

```{r 12_trainbio_fit_build_AKIbio_model_lasso-logi_ROC_nomo}
as_train<-trainbio
names(train)
library(glmnet)
...
```{r 12_trainbio_build_AKIbio_model_lasso-logi_ROC_nomo}
#model
summary(fit.full.step)
library("stargazer")
...
#-------------6.validate the AKI_bio model in test set--------------------------
```{r 13_testbio_validate_AKIbio_model}
as_test<-testbio
...
##############sleduration_0.5+elevatedcrtotreat_12+CR_199+C4_0.08
#-------------7.build AKI_clinc model with logi regression--------------------------

```{r 14_trainnobio_build_AKI_model_lasso-logi_ROC_nomo}
as_train<-trainnobio
fit.full.step<-glm(response ~
...
```{r 15_testnobio_validate_AKIbio_model}
as_test<-testnobio
predictpro<- predict.glm(fit.full.step,type='response',newdata=as_test)
predict =ifelse(predictpro>0.5,1,0)
